...
The operation which consists to open the code source and to display it is considered cheating.
More generally, a program which uses whatever data input cannot be considered a valid quine.
A trivial solution is a program for which the source code is empty.
```
##### Objectives:
```
